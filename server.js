...
app.get('/', (req, res) => {
  db.collection('messages').find().sort({ thumbUp: -1 }).toArray((err, result) => {
    ...
    const groceryList = result.filter(message => message.thumbUp === 0);
    const kitchenInventory = result.filter(message => message.thumbUp > 0);

    res.render('index.ejs', { lists: groceryList, messages: kitchenInventory });
  });
});
